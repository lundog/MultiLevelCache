...
using MultiLevelCaching.Integration.Tests.FakeRedis;
...
namespace MultiLevelCaching.Integration.Tests
{
    ...
    public class SqlTests
    {
        ...
        /// </summary>
        private static string DbHost { get; }

        /// <summary>
        /// Loaded from user secrets.
        /// </summary>
        private static string DbName { get; }

        /// <summary>
        /// Loaded from user secrets.
        /// </summary>
        private static string DbPassword { get; }

        /// <summary>
        /// Loaded from user secrets.
        /// </summary>
        private static string DbUsername { get; }

        static SqlTests()
        {
            var configuration = new ConfigurationBuilder()
                .AddUserSecrets<SqlTests>()
                .Build();
            DbHost = configuration[nameof(DbHost)];
            DbName = configuration[nameof(DbName)];
            DbPassword = configuration[nameof(DbPassword)];
            DbUsername = configuration[nameof(DbUsername)];
        }
        ...
        public async Task GetOrAdd_ManyRequests_Succeeds()
        {
            ...
            var l2Provider = new SqlL2CacheProvider(new() { DbFactory = () => new SqlConnection(connectionString) });
